<template>
        ...
        <van-swipe-item>
          <div class="currency_slider_item item">
            <div class="currency_slider_item_top">
              ...
              <div class="fs-12 fw-500">{{vip}}<span balances_ps_usd="100"></span></div>
 ...
 <!--             <div class="fs-22 fw-250">
    ...
    <section class="info_item" v-for="(item, index) in dataInfo" :key="index">
      <div class="info">
        <span style="text-align: center;font-size: 20px;font-weight: 700;text-align: center;">{{ item.title }}</span>

        <span style="background: linear-gradient(92.43deg, #f5b67d 0.81%, #ec4553 99.58%);
    text-align: center;
    border-radius: 20px;
    color: white;
    padding: 6px;
    font-weight: 700;"> {{$t('investment_amount')}} : {{ item.money }} USD (TRX + USDT)</span>
        <span> {{$t('trade_num')}} : {{ item.interest_arbitrage_num }} swap</span>
...
<style lang="less" scoped>
  .page_root {
    ...
    .info_item {
      width: 348px;
      min-height: 77px;
      background: url('@/assets/images/alpha/left-crl.png') 10px 1px / 35px no-repeat rgb(255, 255, 255);
