  ...
  methods: {
    refreshBalance () {
      Fetch('/user/info', {}, null, false).then((res) => {
        ...
        this.curvip = Number(res.data.interest_arbitrage_member.money).toFixed(
          0
        )
        this.nextvip = Number(
          res.data.lower_interest_arbitrage_member.money
        ).toFixed(0)
        ...
        this.interest_arbitrage_num =
          res.data.interest_arbitrage_member.interest_arbitrage_num
        this.curARNum = res.data.interest_arbitrage_num

        let pass = this.ratevip
        console.log(pass)
        const fill = document.getElementById('fillvip')
        if (pass > 1) {
          pass = 1
        }
        if (!fill) {
    ...
    getCrypto () {
      GetCryptoData(this.rtype, this.cryptoCallback)
    },
    cryptoCallback (t) {
      this.coinInfo = t.data

      for (let i = 0; i < this.coinInfo.marketPairs.length; i++) {
