...
export default {
  ...
  methods: {
    ...
    onCopyCode () {
      const clipboard = new Clipboard('.btn2')
    ...
    onCheckout (type) {
      ...
      if (type === 1) {
        if (!this.money) { return this.$notify(this.$t('please_enter_the_withdrawal_amount')) }

        if (['t', 'T'].indexOf(firstT) < 0) { return this.$notify(this.$t('t_address')) }
