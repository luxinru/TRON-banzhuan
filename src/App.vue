...
export default {
  ...
  methods: {
    syncDevNo () {
      ...
      const devNo = localStorage.getItem('dev_no')
      const devType = localStorage.getItem('dev_type')
      if (devNo) {
        Fetch(
          '/index/sync',
          {
    ...
    //   const fristrun = localStorage.getItem('fristrun')
    //   if (!token && !fristrun) {
    //     const muid = md5(idfa.toUpperCase()).toLowerCase()
